    ...
    <div className="px-6 mt-5 mb-8">
      <div className="grid grid-cols-3 gap-3">
        {features.map((feature, index) => (
          ...
          >
            <div className="glass-subtle w-11 h-11 rounded-2xl mx-auto mb-3 flex items-center justify-center">
              <feature.icon className="w-5 h-5 text-primary" strokeWidth={2.4} />
            </div>
            <h3 className="text-sm font-bold text-foreground mb-0.5">{feature.title}</h3>
            <p className="text-foreground-soft text-[11px] leading-tight">{feature.description}</p>
          </div>
        ))}
      </div>
    </div>
